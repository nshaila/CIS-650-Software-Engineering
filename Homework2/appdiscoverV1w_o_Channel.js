...
box.setContent('(' + my_ip + ') joined Counting Prime with delay. ');
...
function registerCallbacks() {
    ...
    d.on("removed", function(obj) {
        ...
        if (master_ip.localeCompare(obj.address) === 0) {
            masterRemoved = true;
        } else if (isMaster && allNodes !== null && allNodes[currentNode] !== null && allNodes[currentNode].address.localeCompare(obj.address)) {
            allNodes = [];
            console.log("assidgning work again" );
            setTimeout(assignWork, 1000);
        }
    });
...
app.post('/do_post', function(req, res) {
    ...
    if (isMaster) {
        if (parseInt(the_body.work) === 0) {
            box.setContent("Delegating work to all other nodes!!!!!");
            box.style.bg = 'green';
            screen.render();
